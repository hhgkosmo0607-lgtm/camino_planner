...
## 2026-08-03 (44) — 전체 페이지 UI 배치·사용성 개선

**배경**: 색상 파스텔화(43번) 다음으로 "UI 배치도 사용성 좋게 바꿨냐"는 질문에
정직하게 "아니요, 색만 했다"고 답한 뒤 "전체 다 순서대로" 요청받아, 홈부터
`/privacy`까지 페이지를 순서대로 훑으며 레이아웃·정보 구조를 다듬었다. 색상
토큰(43번)과 달리 이번엔 컴포넌트 구조·순서를 실제로 바꿨다.

- **`components/ToolGrid.tsx`(신설)** — 홈 화면 전용. 기존엔 `ToolNav`(9개 도구를
  똑같은 무게의 알약 버튼으로 나열)를 홈에도 그대로 썼는데, 처음 오는 사람에게는
  "이게 뭘 해주는지" 안내가 없었다. 여행 단계(출국 전 준비/현지에서/걸으며
  기록·동행)로 묶고 각 도구에 한 줄 설명을 붙였다. `ToolNav` 자체는 6개 도구
  페이지의 압축 크로스링크 용도로 그대로 둠(다른 문제라 대체 안 함). "일정"
  항목은 바로 위 `CalculatorCTA`와 중복이라 홈에서 제외.
- **`components/RelatedLinks.tsx`(신설)** — "다음으로 가볼 곳" pill 링크 컴포넌트.
  예전엔 페이지 11곳(플랜, 도구 6개, fog, cards, town/stage 템플릿)에 똑같은
  밑줄 텍스트 링크 패턴이 중복돼 있었다 — 전부 이 컴포넌트로 통일. 인쇄 지원이
  필요한 `/tools/timeline`은 `no-print` 래퍼로 감싸 인쇄 시 안 나오게 유지.
- **`/plan`**: 요약(총일수·걸은거리·하루평균·부상위험·콤포스텔라·예산)을
  "이 계획 요약" 소제목 아래 한 덩어리로, 지도·고도단면을 "경로 보기" 아래
  한 덩어리로 묶어 정보 밀도를 낮췄다. 페이지에 `<h1>`이 아예 없던 걸 발견해
  (Mojón 컴포넌트엔 헤딩이 없음, CLAUDE.md가 "임의로 바꾸지 않는다"고 못박아
  건드리지 않음) sr-only `<h1>`을 페이지에 별도로 추가.
- **`/route/[slug]`**: "기간별 추천 일정"(바로 눌러볼 수 있는 카드)을 "전체
  고도 단면"보다 앞으로 옮김 — 실행 가능한 것 먼저, 상세 시각화는 그다음.
- **`/stage/[slug]`**: 3개 숫자 요약을 고도 단면보다 앞으로 옮겨 `/plan`·
  `/route`와 순서를 통일(요약 먼저, 시각화 다음).
- **`/town/[slug]`**: 인접 구간 링크를 `RelatedLinks`로 교체.
- **`/fog`·`/cards`·`/community`**: 상단에 `ToolNav`를 추가해 다른 도구로
  이동하기 쉽게 만들고(이전엔 이 세 페이지만 크로스링크가 없었음), 하단
  링크를 `RelatedLinks`로 통일.
- **`/privacy`**: F-28(커뮤니티) 작업 때 못 채워뒀던 실제 데이터 처리 내용
  (Supabase 계정 분리·닉네임만 공개·신고차단) 섹션을 추가하고, 홈으로
  돌아가는 링크도 새로 달았다(이전엔 페이지 밖으로 나가는 링크가 아예 없었음).

**검증**: `tsc`·`vitest`(139/139)·`eslint`·`next build`(193p, town·stage·route
템플릿 166페이지 포함 전부 정적 생성 성공) 통과 + Playwright 접근성 16페이지
전부 critical/serious 0건 + `plan-share-restore.spec.ts`(이전 세션부터
간헐적으로 실패하던 E2E)도 이번엔 통과 확인(내 변경과 무관한 기존 플레이키니스
였던 것으로 보임) + 홈·`/plan`·`/tools/cost`·`/town/sarria`·`/route/camino-frances`
스크린샷으로 육안 확인.

---

## 2026-08-03 (43) — 디자인 토큰 파스텔화(채도 낮춤)

**배경**: 사용자가 "전체적인 UI를 더 예쁘고 활용성 좋게, 채도를 낮춰 파스텔톤으로"
요청. 두 가지를 먼저 확인했다 — (1) 코발트+노란 화살표(카미노 공식 표지판) 색상군
자체는 유지하고 톤만 부드럽게 할지, 완전히 다른 색상군으로 갈지 → **기존 유지,
톤만 부드럽게**. (2) 접근성(명도대비 4.5:1, 규칙 9)과 파스텔 톤이 충돌할 수
있는데 어느 쪽 우선? → **접근성 절대 준수**.

**색상만 바꾸는 게 왜 전체 화면에 퍼지는가**: `app/globals.css`가 Tailwind v4
`@theme inline`으로 `--ink`·`--sand` 같은 CSS 변수를 `bg-ink`·`text-vino` 같은
Tailwind 클래스에 매핑해뒀다 — 컴포넌트 코드는 전부 이 클래스 이름만 쓰고 hex
값을 직접 안 썼기 때문에, `:root` 블록 10개 값만 바꾸면 앱 전체가 자동으로
따라간다(기존 세션들이 이 패턴을 처음부터 지켜온 덕).

**진행 순서**:
1. WCAG 명도대비 공식을 직접 계산하는 파이썬 스크립트로 후보 색상들을
   검증 — 텍스트로 쓰이는 토큰(ink·text·muted·vino·moss)은 밝게 만들면
   대비가 깨지므로 **채도만 낮추고 명도는 거의 유지**하는 방식으로 설계.
   전부 5.14~14.93:1로 여유 있게 통과(기존 규칙 9의 4.5:1보다 한참 위).
2. Artifact로 실제 화면 요소(모호온 헤더·구간 카드·버튼)를 지금/제안
   나란히 놓은 목업을 만들어 **코드를 건드리기 전에 시각적으로 승인**받음
   (색은 텍스트 설명으로 판단하기 어려워서 — 실제로 렌더된 걸 보고 고르는
   게 더 정확하다고 판단).
3. 승인 후 `app/globals.css` `:root` 10개 값 교체.
4. `components/RouteMap.tsx`(MapLibre GL paint 속성)에 하드코딩된 flecha·
   ink·muted hex 4곳도 같이 교체 — **MapLibre paint 속성은 CSS 변수를 못
   읽어서** globals.css만 고치면 지도 색만 예전 그대로 남는 함정이 있었다
   (grep으로 다른 하드코딩 hex가 더 없는지 전수 확인 완료).

**검증**: `tsc`·`vitest`(139/139)·`eslint`·`next build`(193p) 통과 +
Playwright 접근성 검사 16페이지 **전부 critical/serious 0건**(수식 계산이
아니라 실제 렌더된 페이지에서 axe-core로 재확인) + 홈·`/plan` 스크린샷으로
육안 확인(부드러운 남색 헤더·크림 배경·저채도 골드 강조가 의도대로 나옴).

**⚠️ 팔레트를 또 바꾸게 되면**: `app/globals.css` `:root` + `components/RouteMap.tsx`
4곳(하드코딩 hex, CSS 변수 못 읽음) 둘 다 같이 고칠 것 — 하나만 고치면 지도만
색이 어긋난다.

---

## 2026-08-03 (42) — F-28 동행 매칭 첫걸음(커뮤니티) — 이 프로젝트 첫 DB·계정

**배경**: 사용자가 "커뮤니티 기능도 추가했나?"라고 물었다. 확인해보니 F-28(동행
매칭)·F-27(가족 안심 세트)은 03문서(제품과기능)에 스펙만 있고 코드가 하나도
없었다 — 이 프로젝트가 "DB도 로그인도 없다"는 전제(웹 트랙 Phase 1) 위에서
움직여서다. 사용자가 "진행"을 요청했고, 아키텍처적 중요성(계정·DB가 처음
생기는 결정)을 감안해 **Plan 모드로 범위를 문서화한 뒤 진행**했다.

**범위 확인**: 두 가지를 사용자에게 물어 확정했다 — (1) "가입은 커뮤니티를
쓰고 싶은 사람만" → 계산기 등 핵심 기능은 그대로 무가입 유지, 매칭 자체(1:1
연결)는 "추후"로 명시적으로 미룸. (2) 03문서 Won't 목록 W2("전체공개 피드형
커뮤니티는 제외 — 네이버 카페가 이미 장악")를 부분 반전해 **텍스트 전용
게시판**(사진·좋아요·알고리즘 추천 없음)까지 이번에 추가. 백엔드는 Supabase
(관리형 Postgres+Auth) 선택.

**구현**:
- `supabase/migrations/0001_community.sql` — profiles·checkins·posts·replies·
  reports·blocks 6개 테이블 + RLS 전부. 실명·이메일은 어떤 public 테이블에도
  없다(닉네임만). 차단한 상대의 글은 RLS `NOT EXISTS` 정책으로 로그인한 본인
  화면에서만 걸러진다(비로그인 열람은 그대로 전체 공개).
- `lib/supabase/{client,server,config,types}.ts` — 브라우저/서버 클라이언트
  분리(@supabase/ssr), `isCommunityConfigured()`로 키 없으면 조용히 비활성화
  (`app/api/subscribe/route.ts`의 Resend 패턴과 동일). `types.ts`의 `Database`
  타입에 `Relationships: []`를 빠뜨려 전체가 `never`로 무너지는 걸 처음에
  겪었다 — `@supabase/postgrest-js`의 `GenericTable`이 요구하는 필드였다.
- `lib/community.ts` — 서버 조회 헬퍼(목록·상세, DB 왕복이라 `lib/planner/`
  밖). 쓰기는 각 클라이언트 컴포넌트가 브라우저 클라이언트로 직접 한다.
- `app/community/`(5페이지) + `components/community/`(12개, 전부 `'use
  client'`) — 이메일 매직링크 로그인(만 14세 이상 동의 체크박스), 닉네임
  프로필, "오늘 이 마을에 있어요" 체크인(GPS 좌표 아니라 마을 단위,
  `data/towns.ts` 재사용), 텍스트 게시판(글쓰기·댓글), **신고·차단**(03문서가
  "이 기능의 존재 이유보다 우선한다"고 못박은 필수 안전 기능).
- `proxy.ts`(신설) — Supabase 세션 쿠키 갱신. **Next.js 16이 `middleware.ts`
  파일 관례를 `proxy.ts`로 이름을 바꿨다**는 걸 빌드 경고로 처음 발견해
  즉시 이름을 맞췄다(`export function middleware` → `export function proxy`).
- CLAUDE.md 규칙 8·10에 "커뮤니티는 예외" 명시(핵심 기능 무가입·오프라인
  전제는 계산기에 한정, 부가 기능인 커뮤니티까지 묶는 규칙이 아니었다는 걸
  분명히 함). 기술 스택·디렉토리 구조·주요 기능 표(F-28 행 신설)에도 반영.

**⚠️ 검증 한계 — 로컬에서 실제 로그인·글쓰기·신고·차단까지는 못 봤다**:
`npx supabase start`(Docker 기반 로컬 스택)를 돌리려 했으나, 이 환경의
Windows에 **WSL2 자체가 설치돼 있지 않아**(`wsl --status` → `WSL_E_WSL_
OPTIONAL_COMPONENT_REQUIRED`) Docker Desktop이 영원히 "시작 중" 상태로
멈췄다(`docker ps` → 500 Internal Server Error 계속). `wsl --install
--no-distribution` + 재부팅이 필요한데 이건 시스템 설정 변경이라 사용자
승인 없이 진행하지 않았다. 대신 확인한 것: `tsc`·`vitest`(139/139)·`eslint`·
`next build`(193p, Supabase 키 없이도 성공 — `/community`가 "설정 안 됨"
안내로 우아하게 대체됨)·Playwright 접근성 검사(`/community` 포함 16개
페이지 전부 critical/serious 0건). **실제 매직링크 로그인→글쓰기→신고→차단
흐름은 사용자가 실제 Supabase 프로젝트를 만들고 `.env.local`을 채운 뒤
`npm run dev`로 직접 확인해야 한다.**

**범위 밖(명시적으로 미룸)**: 실제 1:1 매칭 로직·앱 내 메시지(F-28 핵심),
이미지 업로드·좋아요·알고리즘 추천, 자체 관리자 화면(Supabase 대시보드로
수동 처리), 나이 실명 인증(고지만), 이메일 알림.

`plan-share-restore.spec.ts` E2E 1건이 이번 변경과 무관하게 실패 중(체력
"빠르게" 클릭 후 URL이 `f=high`로 안 바뀜) — `app/plan`·`PlanControls.tsx`는
이번 세션에서 손대지 않아 기존 문제로 보인다, 별도 확인 필요.

---

## 2026-08-03 (41) — "실측 필요" 데이터 3건 재조사(forks·albergues·cards A)

**배경**: 지난 세션에서 "실측이 있어야만 채워지는 데이터"로 분류했던 4개 항목
(forks.ts 7/23 variant 거리 null, albergues.ts 4곳 reservation UNKNOWN, F-15
1인칭 체험담, F-12 의료 카드 A 8장)을 왜 막혔는지 사용자에게 설명하고, 4개가
사실 성격이 다르다는 걸 짚었다 — forks·F-15만 진짜 GPS 실측이 필요하고,
albergues는 조사 부족, F-12는 실측이 아니라 의학 자문 게이트였다. 사용자가
"1번은 공개 GPX로, 2번은 재조사로, 3번은 넘어가고, 4번은 초안 작성 후 추후
자문으로" 지시.

**1) forks.ts** — Wikiloc·OutdoorActive 원본 GPX 트랙 페이지는 자동 접근이
막혀 있어(HTTP 403, 봇 차단) 원본 지오메트리 확보는 실패했다. 대신 여러 2차
가이드 사이트(rutasnavarra.com, caminosantiagocompostela.com 등)를 교차
확인하는 방식으로 진행:
- **채움(2건)**: 발투이예(카카벨로스~비야프랑카, 8.6km — caminosantiagocompostela.com이
  구간별 세부 내역 제공, 기존 상충 수치 "+0.4km" 쪽과 일치), 몬테후라(비야투에르타
  변형, 14.6km + 고도 622m/-741m — rutasnavarra.com이 waypoint·고도·소요시간까지
  구체적으로 제시).
- **여전히 null(5건)**: 에우나테 2개(왕복/통과를 섞어 잰 수치들뿐, 우리 정의와
  같은 구간을 잰 출처 없음), 가모날·강변길(부르고스, 하루 전체 거리인지
  도시 진입부만인지 기준 불명 + "비야프리아"와 검색으로 찾은 "카스타냐레스"
  변형이 같은 길인지 미확인), 비아 트라야나(찾은 수치 대부분이 렐리에고스가
  아니라 만시야 데 라스 물라스 기준이라 병합 지점 자체가 다를 가능성).
  항목별 주석에 상세 근거를 남겼다 — 억지로 끼워맞추지 않았다.

**2) albergues.ts** — 벨로라도·토산토스·카스트로헤리스·엘 부르고 라네로 4곳을
Gronze 외 다른 사이트(alberguescaminosantiago.com·consumer.es·caminomaps.org)로
교차 확인해 전부 `reservation: 'NONE'`으로 확정. 부수 발견 2건: 카스트로헤리스
Espacio Interior는 2024년 개업 신규 숙소라 Gronze에 상세 페이지가 없었던 것뿐 —
beds(6)·priceEur(20)도 같이 확보. 엘 부르고 라네로 Domenico Laffi는 **type이
PRIVATE로 잘못 들어가 있었다** — 복수 출처가 전부 "Albergue Municipal"로
표기해 MUNICIPAL로 정정(CLAUDE.md가 경고하는 유형 오분류를 실제로 잡은 사례).

**3) F-15 1인칭 체험담** — 사용자 지시대로 이번엔 건너뜀.

**4) F-12 의료 카드 A(8장)** — 사용자 승인 하에 "먼저 초안 작성, 자문은 나중"
순서로 진행. A1~A6·A8(7장)은 F-04 phrasebook.ts와 똑같은 원칙(전문가에게
도움·추천을 요청하는 데서 멈춘다, 약품명·처치법 없음)으로 스페인어 문장+버튼
스크립트를 작성했다. **자문 검수를 거친 게 아니므로** `blockedReasonKo`에
"검수 대기 중" 문구를 남겼고, `components/CardBrowser.tsx`가 steps와
blockedReasonKo가 둘 다 있는 카드는 경고 배너(⚠, vino색)를 인터랙티브 콘텐츠
위에 띄우도록 수정했다(기존엔 blockedReasonKo가 있으면 무조건 steps를 숨겼는데,
이제 "완전 보류"와 "초안, 검수 대기"를 화면에서도 구분한다). `data/cards.test.ts`
신설 — phrasebook.test.ts와 같은 금칙어(이부프로펜·소염제·파라세타몰·아스피린·
연고·붕대) 목록으로 A축 8장 전체(steps+blockedReasonKo)를 감시. **A7(내 의료
정보)만 성격이 달라 그대로 보류** — 처치 조언이 아니라 순례자 본인의 건강정보
(혈액형·알레르기 등)를 입력해두는 카드라, 필요한 건 의학 자문이 아니라 별도
입력 UI 설계(로컬 저장은 규칙 8 개정으로 이미 허용).

`tsc`·`vitest`(126→139, +13)·`eslint`·`next build`(186p, 신규 라우트 없음)
통과. `next start`+curl로 `/cards` 실제 SSR 확인 — A1 프롬프트·A7 보류 문구
둘 다 정상 노출.
...
F-12는 03문서 5.2절이 "모든 의료 카드는 자문 검수를 거친 뒤에만 배포한다"고
A(부상) 8장 전체를 블록으로 막아뒀지만, B(숙소 9장)·C(이동·기타 6장) 15장은
의료와 무관한 표준 관광 회화 수준이라(F-04 phrasebook.ts가 이미 같은 근거
수준으로 완료돼 있음) 규칙 11과 부딪히지 않는다 — 그래서 15장은 실제 내용까지
채우고 8장만 껍데기로 남겼다.

- **`lib/schema.ts`**: 03문서 5.4절에 "설명용 발췌, 앱 트랙 착수 시점에 반영"
  이라고 미뤄뒀던 `Card`/`CardStep`/`CardOption`/`CardNext` 타입을 이번에
  코드로 옮겼다. `CardNext`에 `NEXT_STEP`/`GOTO_CARD`(흐름형 분기)까지 타입은
  만들어뒀지만 실제로는 전부 `END`만 쓴다 — B1(침대 있나요)·B7(빈대) 원안의
  다단계 분기는 이번 "구조만" 범위에서 뺐다(다음 확장으로 명시).
- **`data/cards.ts`(신설, 23장 고정)**: B1~B9·C1~C6 15장은 promptEs(상대가
  읽는 스페인어)·promptKo(순례자용 작은 안내)·options(상대가 짚는 버튼 4개
  안팎, 결과 한국어 라벨)까지 실제로 채웠다. B3(만실 시 다른 숙소)·C2(버스
  정류장)·C4(길 잃음)는 원안이 지도 짚기·시간 짚기 인터랙션을 요구하는데
  그 UI가 없어서 방향·범위를 묻는 정도로 단순화했다고 주석에 남겼다(정밀
  마을 선택·자동 거리 계산은 다음 확장). A1~A8(부상)은 `steps: []`+
  `blockedReasonKo`만 채웠다 — A7(내 의료 정보)은 정확히는 규칙 11(치료
  조언)이 아니라 혈액형·알레르기 같은 민감한 개인 건강정보를 입력·저장하는
  별도 문제라 이유를 다르게 적었다.
- **`components/CardBrowser.tsx`(신설, `'use client'`) + `app/cards/page.tsx`**:
  카드를 `<details>`로 펼치면 스페인어 문장과 버튼이 뜨고, 버튼을 누르면
  한국어 결과가 그 자리에 표시된다(양방향 — 03문서가 번역기와의 핵심
  차이로 꼽은 부분). 탭 결과는 화면 안에서만 잠깐 보이는 상태라 저장하지
  않는다(개인 기록이 아니라서 규칙 8 localStorage 대상도 아님, 서버로도
  안 보냄). A(부상) 카드는 버튼 없이 `blockedReasonKo` 문구만 보인다.
  `CLAUDE.md` 메뉴명칭표의 "카드"(번역 안 함, 독립 메뉴)를 따라 `/cards`로
  최상위 라우트를 만들었다(`/tools/` 아래 아님). `/plan`에서 링크, sitemap 반영.
- **검증**: 데이터 무결성(23장 고정, id 중복 없음, 카테고리별 개수 9/8/6,
  INJURY는 steps 항상 빈 배열+blockedReasonKo 필수, 나머지는 steps·options
  비어있지 않음) 임시 테스트로 확인 후 삭제. `tsc`·`vitest`(126/126)·
  `eslint`·`next build`(186p, `/cards` 신규) 통과. `next start` + Playwright로
  B1 카드 펼치기→버튼 클릭→한국어 결과 노출, A1 카드는 버튼 없이 보류 문구만
  뜨는 것까지 확인.

---

## 2026-07-31 (38) — F-15 안개 지도 축소판 — 19곳 가이드북 조사 + "여기 도착했어요"

**배경**: F-21 이후 사용자가 F-12·F-15도 실측 없이 만들 수 있는지 재차 물었다.
"이미 실측 지도(OSM+EU-DEM)가 있는데 왜 안 되냐"는 정당한 지적이었다 — 확인해보니
막힌 이유가 지도 데이터가 아니라 **다른 종류의 콘텐츠**였다. F-15는 03문서 원문에
"각 장소가 열리면... 우리가 쓰는 게 아니라 실측 도보에서 직접 보고 쓴다"고 못박혀
있어 **1인칭 체험담**이 필요했던 것이지 좌표·경로가 부족했던 게 아니다. 반면
F-12(보여주기 카드)는 핵심 콘텐츠(약국 카드)가 규칙 11(의료 정보, 자문 검수 전
금지)에 걸려 데이터 출처를 바꿔도 못 만든다 — 성격이 다른 두 가지를 구분했다.

**F-15만 진행**: 이라체 와인 샘·철의 십자가 2곳을 먼저 WebSearch로 조사해
Wise Pilgrim·Follow the Camino·Caminoways 같은 신뢰할 만한 순례 가이드
사이트에서 충분한 역사·유래 정보를 확인, 사용자에게 "19곳 전부 가이드북 출처로
채울 수 있을 것 같다"고 보고하고 승인받아 나머지 17곳까지 조사·완료했다.

- **`lib/schema.ts` `Landmark` 재설계**: 원래 있던 타입은 `lat`/`lng`/`radiusM`
  (실시간 GPS 지오펜싱 전제)·`photoUri`를 갖고 있었는데, 코드 어디서도 쓰인 적
  없어 자유롭게 바꿨다. `km`(towns.ts와 같은 경로상 위치 기준, 좌표 없음 —
  규칙 3과 같은 ODbL 회피 이유 + 지금은 라이브 GPS 자체가 없어서 필요 없음)과
  `source`/`checkedAt`(다른 GUIDEBOOK 데이터들과 같은 정직성 패턴) 추가.
- **`data/landmarks.ts`(신설)**: 19곳 전부. 13곳은 `towns.ts`에 이미 있는
  마을과 정확히 겹쳐 그 km을 그대로 썼고(생장·론세스바예스·팜플로나·푸엔테라레이나·
  아예기·산토도밍고·부르고스·레온·아스토르가·폰페라다·비야프랑카·오세브레이로·
  사리아·포르토마린·몬테도고소), 마을 사이인 4곳(레포에데르 고개·용서의 언덕·
  이라체 와인 샘·철의 십자가)과 상징적 지점 1곳(메세타 시작)은 가이드북에 나온
  대략 거리로 추정했다고 주석에 명시(정밀하지 않음을 숨기지 않음). storyKo는
  전부 역사·유래 요약(가이드북 출처) — 1인칭 체험담이 아니다.
- **`lib/localLog.ts`에 `loadFogState()`/`revealLandmark()` 추가**: F-22·F-23과
  같은 개인 기록 저장(규칙 8) — 스팟 개방 여부·시각을 이 기기에만 남긴다.
- **`components/FogMap.tsx`(신설, `'use client'`) + `app/fog/page.tsx`**: 시각적
  "안개" 그래픽이 아니라 19곳 목록이다 — 이 서비스엔 라이브 내비게이션 지도
  자체가 없어서(06문서 "지도·내비게이션 백엔드"는 Phase 3+ 착수 전), 실시간 GPS로
  자동 개방하는 걸 흉내내지 않고 "여기 도착했어요" 버튼으로 사용자가 직접 연다.
  연 스팟만 이야기가 보이고, 안 연 스팟은 이름·km만 보인다. `/plan`에서 링크,
  `sitemap.ts`에 추가.
- **검증**: 데이터 무결성(id 중복 없음, order 1~19 정확히 한 번씩, townId가
  towns.ts에 실재, km이 경로 진행 방향과 함께 증가) 임시 테스트로 확인 후 삭제
  (data/*.ts는 이 저장소에 전용 테스트 파일 관례가 없어 다른 data 파일들과
  일관되게 유지). `tsc`·`vitest`(126/126)·`eslint`·`next build`(185p, `/fog`
  신규) 통과. `next start` 프로덕션 서버 + Playwright로 "도착했어요" 클릭 →
  이야기 노출 → 새로고침 후에도 유지 확인.
...
채로 남아 있었다. 원안을 그대로 만들려니 "물집·통증 기록"·"매일 지출 입력"처럼
여러 날에 걸쳐 쌓이는 개인 기록이 필요한데, 이건 규칙 8("계획 상태는 URL,
localStorage 금지")과 정면으로 부딪힌다. 사용자에게 트레이드오프(기기 종속·백업
없음 vs URL에 개인정보를 욱여넣는 게 더 나쁜 선택)를 설명하고 **규칙 8을
"계획 상태=URL / 개인 기록=localStorage"로 나누는 개정**을 확인받았다.

- **CLAUDE.md 규칙 8 개정**: 개인 기록(지출·통증 메모·체크리스트 완료 여부)은
  기기 안 localStorage에만 남기고 서버로는 절대 안 보낸다. 다른 기기·브라우저로
  바꾸면 사라진다는 걸 화면에 반드시 명시하는 조건을 달았다.
- **`lib/cost.ts`(신설, 순수 함수)**: `/tools/cost`가 쓰던 `computeCost`를
  `estimateTripCostManwon`으로 추출해 공유(F-06와 F-23이 같은 계산을 쓰게 함,
  중복 방지). `summarizeExpenses()`(지출 합산)도 여기. 테스트 7개.
- **`lib/localLog.ts`(신설)**: localStorage 읽기·쓰기 헬퍼. `lib/planner/`가
  아니라 밖에 둔 이유는 window를 참조해서 순수 함수가 아니기 때문(규칙 6은
  `lib/planner/`만 대상). SSR 중엔 빈 값 반환, try/catch로 저장 공간 초과 등을
  조용히 무시(개인 기록 실패로 화면 전체를 막지 않음). 저장 키는 `dayNo`가 아니라
  마을쌍(`fromTownId>toTownId`)— 일정 설정을 바꿔도(하루 km 등) 기록이 안 흔들리게.
- **`components/DayChecklist.tsx`(신설, `'use client'`)**: `StageCard` 안
  "오늘 체크리스트" 접힘. 아침 4항목(테이핑·빨래·배터리·크레덴시알, 고정)+
  물 위치·위험구간(그 Stage의 실제 `waypoints`/`hazards`에서 자동 생성, 없으면
  항목 자체를 뺌 — 지어내지 않음), 저녁 4항목(발 확인·빨래·도장·내일 숙소 확인)+
  통증 메모(자유 텍스트, "자동으로 일정이 조정되지 않는다"고 명시 — F-21과 같은
  과장 금지 원칙)+지출 입력(카테고리·금액·메모). 체크박스는 서버 렌더 시 빈
  상태로 그리고 마운트 후에만 localStorage 값으로 채운다(하이드레이션 불일치
  방지 — 새 eslint 규칙 `react-hooks/set-state-in-effect`가 이 패턴을 경고해서
  이유를 주석으로 남기고 그 줄만 껐다).
- **`components/BudgetSummary.tsx`(신설, `'use client'`)**: `/plan` 상단에
  "내 지출 기록" — 이 기기에 기록된 모든 날의 지출을 모아 카테고리별 합계·계획
  예산(공립·외식 50%씩 가정한 대략치) 대비 %를 보여준다. 기록 없으면 안내
  문구만. "기록 전체 지우기" 버튼 포함(confirm 후 `localStorage` 통째 삭제).
  절약 팁 제안(03문서 원안)은 만들지 않음 — 남은 일수·카테고리별 절감액 추정은
  근거 없는 가중치를 또 만드는 셈이라 규칙 11 정신에 어긋난다고 판단.
- **범위 밖으로 명시적으로 남긴 것**: 배낭 최종 점검(이미 `/tools/pack`이 함),
  완주 후 마무리 체크리스트(Phase 3 완주 화면 필요), "가족에게 도착 알림"
  (E축 알림 인프라 없음), CSV·PDF 내보내기.
- **검증**: `tsc`·`vitest`(114/114)·`eslint`·`next build`(184p) 통과. `next
  start` 프로덕션 서버 + Playwright(스크립트, 임시 파일로 실행 후 삭제)로 실제
  체크박스 토글→새로고침 후 유지, 지출 추가→새로고침 후 유지, `BudgetSummary`
  합계 반영까지 확인. 콘솔 에러 없음(Vercel Analytics 스크립트 404는 로컬
  환경 특성상 원래 발생하는 것, 기존에도 있던 것).

---

## 2026-07-31 (35) — F-21 Plan B 축소판 — "이 날이 어긋나면" 미리보기

**배경**: F-21은 CLAUDE.md 기능표에서 Phase 3(F-12·F-15와 함께 앱 트랙 검증 이후)로
분류돼 있다. 03문서 원안은 상황 선택 → 대안 목록 → "적용하고 이후 일정 재계산"까지
포함하는 큰 기능(뒤 구간 거리·날짜·콤포스텔라 요건 전체 재검증)이라, 그대로 만들면
Phase 경계를 넘는다(CLAUDE.md가 명시적으로 경계하는 실수 — F-19 이전에도 한 번
있었다). **사용자에게 범위를 먼저 확인해 "축소판"으로 합의**: 대안 미리보기만
카드에 붙이고, 실제 반영·재계산은 다음 단계로 명시적으로 남긴다.

- **`lib/planner/planB.ts`(신설, 순수 함수)**: `planBOptions(fromTownId, toTownId)`
  — 그날 하루 기준으로 4종 대안을 계산한다.
  - **더 간다**: 다음 마을까지(있으면). 총 거리·증감·예상 소요·침대, `LONG_DISTANCE`
    (28km, split.ts와 동일 기준) 초과 시 경고.
  - **되돌아간다**: 이전 마을까지. `toIdx - 1 === fromIdx`(바로 다음 마을이 원래
    도착지)면 대안 자체가 없음(하루 이동 0km는 대안이 아니라서 뺀다).
  - **이동수단**: `findTransitOptions()`로 실제 조사된 노선(F-26, 현재 부르고스~
    레온·사아군~레온)이 있는 구간에서만 뜬다 — 없으면 지어내지 않고 옵션을
    아예 뺀다(규칙 1).
  - **사립 예약**: 목적지에 `PRIVATE` 타입 알베르게가 있으면 개수만 안내, 없으면
    옵션 없음.
  - `accumulateProfile`/`estimatedMinutes`를 재사용해 오르막·소요시간을 실제
    지형 기준으로 계산한다(regular Stage와 같은 정확도).
- **`components/PlanBPanel.tsx`(신설)**: `StageCard` 맨 아래에 `<details>`로
  "이 날이 어긋나면" 접힘(JS 없이도 펼쳐짐, 규칙 7). **"미리보기입니다 — 선택해도
  이후 일정이 자동으로 다시 계산되지는 않습니다"를 화면에 명시** — 실제로 반영된
  것처럼 과장하지 않는다.
- `lib/planner/split.ts`의 `townIdx`를 `export`로 바꿔 `planB.ts`가 재사용하게 함.
- 테스트 5개 추가(부르고스→오르니요스 실제 데이터로 4종 옵션 검증, 경계 케이스
  2개, 이동수단 있음/없음 대조). 총 107/107.
- **남은 것(다음 단계, 아직 안 함)**: "상황을 고르세요"(자리 없음/몸 안좋음/
  늦게 출발/날씨) UI, "적용" 시 URL에 오버라이드를 담아 이후 구간을 실제로
  다시 나누는 재계산(skip/variantChoices와 같은 패턴으로 확장 가능해 보임),
  "직접 고치기"(마을 직접 입력), 자동 감지(숙소 폐쇄·계절 폐쇄·통증 기록 등).

`tsc`·`vitest`(107/107)·`eslint`·`next build`(184p, 신규 라우트 없음) 통과.
`npm run dev`로 `/plan?d=21&sd=2026-09-01` curl SSR 확인 — 여러 날짜 카드에서
더 간다·되돌아간다·사립예약 문구 정상 노출.
